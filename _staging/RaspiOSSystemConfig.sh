...
alias mode='_m() { [ -z "$1" ] && (echo -n "The system is currently in "; systemctl -q is-active mame-autostart.service && echo -n ARCADE || echo -n SERVICE; echo " mode.") || if [[ "${1,,}" =~ ^(arcade|service)$ ]]; then case "${1,,}" in arcade) sudo systemctl enable mame-autostart.service ;; service) sudo systemctl disable mame-autostart.service ;; esac; else echo "Usage: mode [arcade | service]"; fi; }; _m'
EOF
